...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.pakalon.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `pakalon.settings.dat` is special: it maps to the `pakalon.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"pakalon.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // pakalon.settings.dat → the electron settings store ("pakalon.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "pakalon.global.dat").
  const storeName = filename === "pakalon.settings.dat" ? "pakalon.settings" : filename
