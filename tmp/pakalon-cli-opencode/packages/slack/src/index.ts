...
import { createOpencode, type ToolPart } from "@pakalon-ai/sdk"
...
console.log("🚀 Starting pakalon server...")
const pakalon = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await pakalon.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new pakalon session...")
    const { client, server } = pakalon
    ...
    console.log("✅ Created pakalon session:", createResult.data.id)
  ...
  console.log("📝 Sending to pakalon:", message.text)
