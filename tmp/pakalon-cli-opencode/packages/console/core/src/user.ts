  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@pakalon-ai/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on Pakalon`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.inviterEmail,
              assetsUrl: `https://pakalon.ai/email`,
