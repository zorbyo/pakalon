...
Replace runtime Bun shell template-tag usage in `packages/pakalon/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/pakalon/src` is removed except:
