...
test("parseModel normalizes whitespace around free-model suffixes", () => {
  const result = Provider.parseModel("pakalon/nvidia/nemotron-3-super-120b-a12b: free")
  expect(String(result.providerID)).toBe("pakalon")
  expect(String(result.modelID)).toBe("nvidia/nemotron-3-super-120b-a12b:free")
})

test("getModel accepts normalized pakalon model IDs with stray whitespace", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "pakalon.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const model = await Provider.getModel(
        ProviderID.pakalon,
        ModelID.make("nvidia/nemotron-3-super-120b-a12b: free"),
      )
      expect(String(model.providerID)).toBe("pakalon")
      expect(String(model.id)).toBe("nvidia/nemotron-3-super-120b-a12b:free")
    },
  })
})

test("getModel maps legacy pakalon provider selections to openrouter", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "pakalon.json"),
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "pakalon" },
              },
            },
          },
        }),
