...
import type { Event, EventMessagePartUpdated, ToolStatePending, ToolStateRunning } from "@pakalon-ai/sdk/v2"
...
function createFakeAgent() {
  ...
  const agent = new ACP.Agent(connection, {
    sdk,
    defaultModel: { providerID: "pakalon", modelID: "big-pickle" },
  } as any)
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "/tmp/pakalon-acp-test"
