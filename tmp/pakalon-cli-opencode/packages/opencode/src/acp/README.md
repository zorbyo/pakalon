...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for pakalon.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal pakalon sessions
...
pakalon acp --cwd /path/to/project
```
...
```bash
PAKALON_ENABLE_QUESTION_TOOL=1 pakalon acp
```
...
- **Terminal Integration**: Full terminal support via pakalon's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | pakalon acp
```
...
ACP sessions map cleanly to pakalon's internal session model:
