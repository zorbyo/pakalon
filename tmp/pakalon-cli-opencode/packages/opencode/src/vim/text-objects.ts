...
/**
 * Check if character is vim whitespace
 */
...
/**
 * Check if character is vim punctuation (non-word, non-whitespace)
 */
...
/**
 * Get grapheme segmenter for proper Unicode handling
 */
function* segmentGraphemes(
  text: string
): Generator<{ segment: string; index: number }> {
  // Simple grapheme segmentation - in production use Intl.Segmenter
  let index = 0
