...
/**
 * Extended cursor interface for transitions
 */
interface TransitionCursor extends Cursor {
  isAtEnd(): boolean
...
function handleNormalInput(
  ...
  if (input === "G") {
    return {
      execute: () => {
        const cursor = ctx.cursor as TransitionCursor
        // count=1 means no count given, go to last line
        // otherwise go to line N
        if (count === 1) {
          ctx.setOffset(cursor.startOfLastLine().offset)
        } else {
          ctx.setOffset(cursor.goToLine(count).offset)
        }
      },
    }
  }
  ...
  if (input === "I") {
    ...
    return {
      execute: () =>
        ctx.enterInsert(cursor.firstNonBlankInLogicalLine().offset),
    }
  }
  if (input === "a") {
    const cursor = ctx.cursor as TransitionCursor
    return {
      execute: () => {
        const newOffset = cursor.isAtEnd()
          ? cursor.offset
          : cursor.right().offset
...
function fromFind(
  ...
  return {
    execute: () => {
      ...
      const result = cursor.findCharacter(input, state.find, state.count)
      if (result !== null) {
...
function fromG(
  ...
  if (input === "g") {
    ...
    return {
      execute: () => ctx.setOffset(cursor.startOfFirstLine().offset),
    }
  }
  return { next: { type: "idle" } }
}
...
function executeRepeatFind(
  ...
  const result = cursor.findCharacter(lastFind.char, findType, count)
  if (result !== null) {
    ctx.setOffset(result)
  }
}
