...
export function Autocomplete(props: {
  ...
  const [store, setStore] = createStore({
    ...
    lastFileSelectedAt: 0 as number,
  })
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    // Extract command name from display string for deduplication
    const getCommandName = (display: string): string => {
      const trimmed = display.trim().toLowerCase()
      // Handle /command:mcp format
      const colonIndex = trimmed.indexOf(":")
      if (colonIndex > 0) return trimmed.slice(0, colonIndex)
      // Handle aliases like /sessions and /session
      return trimmed.split(" ")[0] ?? trimmed
    }

    const addUnique = (option: AutocompleteOption) => {
      const normalizedDisplay = (option.display ?? "").trim().toLowerCase()
      if (!normalizedDisplay) return

      const commandName = getCommandName(normalizedDisplay)
      if (seen.has(commandName)) return
      seen.add(commandName)
      results.push(option)
    }

    for (const slash of command.slashes()) {
      addUnique(slash)
    }
  ...
  function select() {
    ...
    if (!selected) return
    hide()
    // Set timestamp when a file (not directory) is selected
    // This is used to implement double-Enter behavior:
    // First Enter: select file, Second Enter: submit prompt
    if (!selected.isDirectory) {
      setStore("lastFileSelectedAt", Date.now())
    }
    selected.onSelect?.()
  }
    ...
    props.ref({
      ...
      onKeyDown(e: KeyEvent) {
        if (store.visible) {
          ...
          if (name === "return") {
            const now = Date.now()
            const wasFileSelectedRecently = now - store.lastFileSelectedAt < 500

            if (wasFileSelectedRecently) {
              // Second Enter after file selection - reset tracking and let prompt submit
              setStore("lastFileSelectedAt", 0)
              e.preventDefault()
              return
            }

            // First Enter - select the file (don't submit)
            select()
            e.preventDefault()
            return
          }
