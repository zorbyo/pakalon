...
import pakalon from "./theme/pakalon.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (config.theme ?? kv.get("theme", "pakalon")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.pakalon, store.mode)
    })
