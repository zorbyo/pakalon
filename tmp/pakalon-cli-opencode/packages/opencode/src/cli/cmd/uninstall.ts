...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall pakalon and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Pakalon")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g pakalon-ai",
      pnpm: "pnpm uninstall -g pakalon-ai",
      bun: "bun remove -g pakalon-ai",
      yarn: "yarn global remove pakalon-ai",
      brew: "brew uninstall pakalon",
      choco: "choco uninstall pakalon",
      scoop: "scoop uninstall pakalon",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "pakalon", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Pakalon!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# pakalon") || content.includes(".pakalon/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".pakalon/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".pakalon/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".pakalon"))
    ) {
      continue
    }
