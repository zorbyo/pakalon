    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: pakalon mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in pakalon.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .pakalon/ subdirectory too)
  const candidates = [path.join(baseDir, "pakalon.json"), path.join(baseDir, "pakalon.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".pakalon", "pakalon.json"), path.join(baseDir, ".pakalon", "pakalon.jsonc"))
  }
  ...
  // Default to pakalon.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., pakalon x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "pakalon-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
