...
export namespace MCP {
  ...
  export const Status = z
    .discriminatedUnion("status", [
      ...
      z
        .object({
          status: z.literal("not_initialized"),
        })
        .meta({
          ref: "MCPStatusNotInitialized",
        }),
    ])
    .meta({
      ref: "MCPStatus",
  ...
  const state = Instance.state(
    async () => {
      // Don't auto-connect to MCP servers at startup - lazy loading
      // Connection happens when connect() is explicitly called
      return {
        status: {} as Record<string, Status>,
        clients: {} as Record<string, MCPClient>,
      }
    },
    async (state) => {
        ...
        try {
          ...
          if (isAuthError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              Bus.publish(TuiEvent.ToastShow, {
                title: "MCP Authentication Required",
                message: `Server "${key}" requires authentication. Run: pakalon mcp auth ${key}`,
                variant: "warning",
                duration: 8000,
              }).catch((e) => log.debug("failed to show toast", { error: e }))
            }
  ...
  export async function status() {
    ...
    for (const [key, mcp] of Object.entries(config)) {
      ...
      // If connected (status is in state), use it; otherwise use config-based status
      if (s.status[key]) {
        result[key] = s.status[key]
      } else if (mcp.enabled === false) {
        result[key] = { status: "disabled" }
      } else {
        result[key] = { status: "not_initialized" }
      }
    }
