...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("pakalon://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("pakalon://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("pakalon://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("pakalon://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("pakalon://open-project")).toBeUndefined()
    expect(parseDeepLink("pakalon://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "pakalon://open-project?directory=/a",
      "pakalon://other?directory=/b",
      "pakalon://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("pakalon://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("pakalon://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("pakalon://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("pakalon://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "pakalon://new-session?directory=/a",
      "pakalon://open-project?directory=/b",
      "pakalon://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __PAKALON__: {
        deepLinks: ["pakalon://open-project?directory=/a"],
      },
    } as unknown as Window & { __PAKALON__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["pakalon://open-project?directory=/a"])
