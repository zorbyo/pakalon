...
import type { Message } from "@pakalon-ai/sdk/v2/client"
...
describe("getSessionContextMetrics", () => {
  ...
  test("recomputes when message array is mutated in place", () => {
    ...
    expect(two.totalTokens).toBe(120)
  })
  ...
  test("excludes summary assistant messages from totals", () => {
    const normal = assistant("a1", { input: 30, output: 20, reasoning: 0, read: 0, write: 0 }, 0.4)
    const summary = {
      ...assistant("a2", { input: 300, output: 200, reasoning: 0, read: 0, write: 0 }, 1.2),
      summary: true,
    } as unknown as Message

    const metrics = getSessionContextMetrics([user("u1"), normal, summary], [{ id: "openai", models: {} }])

    expect(metrics.totalCost).toBe(0.4)
    expect(metrics.totalTokens).toBe(50)
