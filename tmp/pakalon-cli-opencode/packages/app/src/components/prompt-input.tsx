import { useFilteredList } from "@pakalon-ai/ui/hooks"
...
import { DockShellForm, DockTray } from "@pakalon-ai/ui/dock-surface"
...
import { ImagePreview } from "@pakalon-ai/ui/image-preview"
...
// Handle slash command insert events (e.g., /web command)
onMount(() => {
  const handleSlashCommandInsert = (e: Event) => {
  ...
  window.addEventListener("slash-command-insert", handleSlashCommandInsert)
  onCleanup(() => window.removeEventListener("slash-command-insert", handleSlashCommandInsert))
})
  ...
  const thinkingEnabled = createMemo(
    () => local.model.variant.list().length > 0 && (local.model.variant.current() ?? undefined) !== undefined,
  )
  const toggleThinkingMode = () => {
    const available = local.model.variant.list()
    if (available.length === 0) return false
    local.model.variant.set(thinkingEnabled() ? undefined : available[0])
    return true
  }
  const promptText = () => prompt.current().map((part) => ("content" in part ? part.content : "")).join("")
  const lineBoundary = (position: number, edge: "start" | "end") => {
    const text = promptText()
    if (!text) return 0
    const clamped = Math.max(0, Math.min(position, text.length))
    const lineStart = text.lastIndexOf("\n", Math.max(0, clamped - 1)) + 1
    if (edge === "start") return lineStart
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (!event.altKey && !event.ctrlKey && !event.metaKey && (event.key === "Home" || event.key === "End")) {
      const position = lineBoundary(getCursorPosition(editorRef), event.key === "Home" ? "start" : "end")
    ...
    if (!event.shiftKey && !event.altKey && !event.ctrlKey && !event.metaKey && event.key === "Tab") {
      if (toggleThinkingMode()) {
        event.preventDefault()
      }
      return
    }
    ...
    <div class="relative size-full _max-h-[320px] flex flex-col gap-0">
      ...
      <DockShellForm
        ...
        style={{
          ...
          "border": "none",
          "border-top": "2px solid #E8AA41",
          "border-bottom": "2px solid #E8AA41",
          "border-radius": "4px",
        }}
      >
        <PromptDragOverlay
        ...
        >
          ...
          >
            <div
              ...
              classList={{
                "select-text": true,
                "w-full pl-3 pr-2 pt-2 text-14-regular text-white focus:outline-none whitespace-pre-wrap": true,
              ...
              <div class="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                <div data-component="prompt-variant-control">
                  <div class="flex items-center gap-2">
                    ...
                    <div class="hidden sm:flex items-center min-w-[92px] text-11-medium uppercase tracking-[0.08em] text-text-weak">
