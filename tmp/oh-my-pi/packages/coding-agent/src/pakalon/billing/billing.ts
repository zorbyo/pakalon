...
const DEFAULT_MODEL_PRICING: Record<string, ModelPricing> = {
	"openai/gpt-4.1": { inputPerMillionUsd: 2.0, outputPerMillionUsd: 8.0 },
	"openai/gpt-4.1-mini": { inputPerMillionUsd: 0.4, outputPerMillionUsd: 1.6 },
	"anthropic/claude-3-5-sonnet": { inputPerMillionUsd: 3.0, outputPerMillionUsd: 15.0 },
	"anthropic/claude-3-5-haiku": { inputPerMillionUsd: 0.8, outputPerMillionUsd: 4.0 },
	"anthropic/sonnet-4.6": { inputPerMillionUsd: 3.0, outputPerMillionUsd: 15.0 },
	"openrouter/sonnet-4.6": { inputPerMillionUsd: 3.0, outputPerMillionUsd: 15.0 },
	"openrouter/gpt-5.3-codex": { inputPerMillionUsd: 1.75, outputPerMillionUsd: 14.0 },
	"gpt-5.3-codex": { inputPerMillionUsd: 1.75, outputPerMillionUsd: 14.0 },
};

export function initializeBilling(baseDir?: string): BillingInfo {
	const stored = loadBillingConfig(baseDir);
	...
	usageHistory = stored?.usageHistory ?? loadUsageHistory(baseDir);
	invoices = stored?.invoices ?? loadInvoices(baseDir);
	...
	currentBilling.billingCycles = stored?.billingCycles ?? loadBillingCycles(baseDir);
	saveBillingConfig(currentBilling, baseDir);
...
function ensureBillingState(baseDir?: string): BillingInfo | null {
	if (!currentBilling) {
		return initializeBilling(baseDir);
	}
	return currentBilling;
}
...
export function calculateModelUsageCost(
	...
	inputTokens: number,
	outputTokens: number,
	pricing: ModelPricing = DEFAULT_MODEL_PRICING[modelId] ?? {
...
export async function trackUsage(
	...
	const modelCost =
		modelId && inputTokens !== undefined && outputTokens !== undefined
...
export async function getCurrentCycleUsage(): Promise<BillingCycleUsage> {
	...
	const records = usageHistory.filter(
		record => record.timestamp >= startDate.getTime() && record.timestamp <= endDate.getTime(),
	);
...
export function upgradePlan(planId: string): BillingInfo {
	const plans: Record<string, BillingPlan> = {
		...
		team: {
			...
			name: "Team",
			price: 40,
			features: ["team-sharing", "admin-panel", "unlimited-tokens"],
...
export async function trackUsageAndBillReminder(email: string, amount: number, daysUntilDue: number): Promise<void> {
	const { sendBillingReminder } = await import("./email");
	await sendBillingReminder(email, daysUntilDue, amount);
}

export async function sendBillingReminderNotification(
	email: string,
	daysUntilDue: number,
	amount: number,
): Promise<void> {
...
export async function sendBillingNotification(type: string, data: unknown): Promise<void> {
	...
	const { sendBillingEmail } = await import("./email");
