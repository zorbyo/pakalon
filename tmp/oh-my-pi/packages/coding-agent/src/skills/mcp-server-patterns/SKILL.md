...
- **Prompts**: Reusable, parameterised prompt templates the client can surface (e.g. in Pakalon Desktop). Register with `registerPrompt()` or equivalent.
- **Transport**: stdio for local clients (e.g. Pakalon Desktop); Streamable HTTP is preferred for remote (Cursor, cloud). Legacy HTTP/SSE is for backward compatibility.
