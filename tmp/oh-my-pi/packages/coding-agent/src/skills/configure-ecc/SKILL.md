---
name: configure-Pakalon
description: Interactive installer for Everything Pakalon — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: Pakalon
---

# Configure Everything Pakalon (Pakalon)

An interactive, step-by-step installation wizard for the Everything Pakalon project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure Pakalon", "install Pakalon", "setup everything Pakalon", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing Pakalon installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Pakalon before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install everything-Pakalon-code` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.Pakalon/skills/configure-Pakalon/SKILL.md`, then activate by saying "configure Pakalon"

---

## Step 0: Clone Pakalon Repository

Before any installation, clone the latest Pakalon source to `/tmp`:

```bash
rm -rf /tmp/everything-Pakalon-code
git clone https://github.com/affaan-m/everything-Pakalon-code.git /tmp/everything-Pakalon-code
```

Set `Pakalon_ROOT=/tmp/everything-Pakalon-code` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing Pakalon clone.
...
```
Question: "Where should Pakalon components be installed?"
Options:
  - "User-level (~/.Pakalon/)" — "Applies to all your Pakalon projects"
  - "Project-level (.Pakalon/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.Pakalon`
- Project-level: `TARGET=.Pakalon` (relative to current project root)
- Both: `TARGET_USER=~/.Pakalon`, `TARGET_PROJECT=.Pakalon`
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, Pakalon API patterns"
...
| `Pakalon-api` | Anthropic Pakalon API patterns: Messages, streaming, tool use, vision, batches, Agent SDK |
...
```bash
cp -r $Pakalon_ROOT/skills/<skill-name> $TARGET/skills/
```
...
cp -r $Pakalon_ROOT/rules/common/* $TARGET/rules/

# Language-specific rules (flat copy into rules/)
cp -r $Pakalon_ROOT/rules/typescript/* $TARGET/rules/   # if selected
cp -r $Pakalon_ROOT/rules/python/* $TARGET/rules/        # if selected
cp -r $Pakalon_ROOT/rules/golang/* $TARGET/rules/        # if selected
```
...
```bash
grep -rn "~/.Pakalon/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.Pakalon/` paths:
- If a skill references `~/.Pakalon/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.Pakalon/skills/` or `~/.Pakalon/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.Pakalon/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.Pakalon/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .Pakalon/skills/")
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source Pakalon repository (`$Pakalon_ROOT/`).
...
### "Skills not being picked up by Pakalon"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.Pakalon/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.Pakalon/skills/<skill-name>/SKILL.md` exists
...
- Restart Pakalon after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.Pakalon/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.Pakalon/homunculus/` directory is always user-level — this is expected and not an error.
