...
# Pakalon passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.Pakalon/settings.json.
...
# Read JSON from stdin (Pakalon hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  export Pakalon_PROJECT_DIR="$STDIN_CWD"
fi
...
CONFIG_DIR="${HOME}/.Pakalon/homunculus"
...
#   - Pakalon observing its own Haiku observer sessions (self-loop)
#   - Pakalon observing other tools' automated sessions
...
# (Pakalon_HOOK_PROFILE=minimal, Pakalon_SKIP_OBSERVE=1, agent_id, path exclusions).
case "${Pakalon_CODE_ENTRYPOINT:-cli}" in
  cli|sdk-ts) ;;
  *) exit 0 ;;
esac

# Layer 2: minimal hook profile suppresses non-essential hooks.
[ "${Pakalon_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${Pakalon_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_Pakalon_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_Pakalon_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_Pakalon_SKIP_PATHS="${Pakalon_OBSERVE_SKIP_PATHS:-observer-sessions,.Pakalon-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _Pakalon_SKIP_ARRAY <<< "$_Pakalon_SKIP_PATHS"
  for _pattern in "${_Pakalon_SKIP_ARRAY[@]}"; do
...
# Pass HOOK_PHASE via env var since Pakalon does not include hook type in stdin JSON
...
try:
    ...
    # Pakalon does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - Pakalon hook format
...
# which caused runaway parallel Pakalon analysis processes.
SIGNAL_EVERY_N="${Pakalon_OBSERVER_SIGNAL_EVERY_N:-20}"
