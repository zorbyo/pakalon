...
def project_tree(tmp_path):
    ...
    homunculus = tmp_path / ".Pakalon" / "homunculus"
...
def test_detect_project_global_fallback(patch_globals, monkeypatch):
    ...
    monkeypatch.delenv("Pakalon_PROJECT_DIR", raising=False)
...
def test_detect_project_from_env(patch_globals, monkeypatch, tmp_path):
    """Pakalon_PROJECT_DIR env var should be used as project root."""
    fake_repo = tmp_path / "my-repo"
    fake_repo.mkdir()
    monkeypatch.setenv("Pakalon_PROJECT_DIR", str(fake_repo))
