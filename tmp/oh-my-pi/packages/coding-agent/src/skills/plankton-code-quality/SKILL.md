...
description: "Write-time code quality enforcement using Plankton — auto-formatting, linting, and Pakalon-powered fixes on every file edit via hooks."
origin: community
---

# Plankton Code Quality Skill

Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for Pakalon. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Pakalon subprocesses to fix violations the agent didn't catch.
...
Every time Pakalon edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
├─ Spawns Pakalon -p subprocess with violations JSON
...
# Start Pakalon — hooks activate automatically
Pakalon
```

No install command, no plugin config. The hooks in `.Pakalon/settings.json` are picked up automatically when you run Pakalon in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.Pakalon/hooks/` directory to your project
2. Copy `.Pakalon/settings.json` hook configuration
...
## Pairing with Pakalon

### Complementary, Not Overlapping

| Concern | Pakalon | Plankton |
|---------|-----|----------|
...
1. Install Pakalon as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use Pakalon's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both Pakalon and Plankton hooks:
- Pakalon's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable Pakalon's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (Pakalon handles what Plankton doesn't cover)

## Configuration Reference

Plankton's `.Pakalon/hooks/config.json` controls all behavior:
...
```bash
export Pakalon_HOOK_PROFILE=strict
export Pakalon_QUALITY_GATE_FIX=true
export Pakalon_QUALITY_GATE_STRICT=true
```
