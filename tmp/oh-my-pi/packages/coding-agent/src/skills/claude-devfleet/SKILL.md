---
name: Pakalon-devfleet
description: Orchestrate multi-agent coding tasks via Pakalon DevFleet — plan projects, dispatch parallel agents in isolated worktrees, monitor progress, and read structured reports.
origin: community
---

# Pakalon DevFleet Multi-Agent Orchestration

## When to Use

Use this skill when you need to dispatch multiple Pakalon agents to work on coding tasks in parallel. Each agent runs in an isolated git worktree with full tooling.

Requires a running Pakalon DevFleet instance connected via MCP:
```bash
Pakalon mcp add devfleet --transport http http://localhost:18801/mcp
```
