...
# Runs on Stop hook to extract reusable patterns from Pakalon sessions
#
# Why Stop hook instead of UserPromptSubmit:
# - Stop runs once at session end (lightweight)
# - UserPromptSubmit runs every message (heavy, adds latency)
#
# Hook config (in ~/.Pakalon/settings.json):
# {
#   "hooks": {
#     "Stop": [{
#       "matcher": "*",
...
# Extracted skills saved to: ~/.Pakalon/skills/learned/

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/config.json"
LEARNED_SKILLS_PATH="${HOME}/.Pakalon/skills/learned"
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.Pakalon/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (Pakalon hook input)
...
if [ -z "$transcript_path" ]; then
  transcript_path="${Pakalon_TRANSCRIPT_PATH:-}"
fi
...
# Signal to Pakalon that session should be evaluated for extractable patterns
