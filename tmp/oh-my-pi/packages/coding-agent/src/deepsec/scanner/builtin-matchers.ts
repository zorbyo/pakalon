/**
 * Deepsec Built-in Matcher Registry — all security vulnerability matchers.
 */
import type { MatcherPlugin } from "../core/types";
	...
	match(content: string, filePath: string) {
		...
		return regexMatcher(
			...
			[
				...
				{
					regex: /(password|passwd|secret|api_key|apikey|api[-_]secret)\s*[:=]\s*['"][^'"]{8,}['"]/i,
