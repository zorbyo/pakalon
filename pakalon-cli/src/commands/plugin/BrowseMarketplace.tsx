...
export function BrowseMarketplace({
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      // All succeeded
      const message = `[OK] Installed ${successCount_0} ${plural(successCount_0, 'plugin')}. ` + `Run /reload-plugins to activate.`;
      setResult(message);
      ...
      const message_0 = `[OK] Installed ${successCount_0} of ${successCount_0 + failureCount} plugins. ` + `Failed: ${formatFailureDetails(newFailedPlugins, false)}. ` + `Run /reload-plugins to activate successfully installed plugins.`;
      setResult(message_0);
    }
  ...
  if (typeof viewState === 'object' && viewState.type === 'plugin-options') {
    ...
    return <PluginOptionsFlow plugin={plugin_5} pluginId={pluginId_2} onDone={(outcome, detail) => {
      switch (outcome) {
        case 'configured':
          finish(`[OK] Installed and configured ${plugin_5.name}. Run /reload-plugins to apply.`);
          break;
        case 'skipped':
          finish(`[OK] Installed ${plugin_5.name}. Run /reload-plugins to apply.`);
          break;
        case 'error':
