  ...
  const handleSelectIDE = (value: string) => {
    if (value !== 'None' && shouldShowAutoConnectDialog()) {
      ...
      if (value === 'None' && shouldShowDisableAutoConnectDialog()) {
        ...
        onSelect(
          availableIDEs.find(ide => ide.port === parseInt(value)),
        )
      }
    }
  }

  const ideCounts = availableIDEs.reduce<Record<string, number>>(
    (acc, ide) => {
...
function RunningIDESelector({
  ...
  onSelectIDE,
  onDone,
}: RunningIDESelectorProps): React.ReactNode {
  const [selectedValue, setSelectedValue] = useState(runningIDEs[0] ?? '')

  const handleSelectIDE = (value: IdeType) => {
    onSelectIDE(value)
  }
...
function InstallOnMount({ ide, onInstall }: InstallOnMountProps): null {
  useEffect(() => {
    onInstall(ide)
  }, [ide, onInstall])
...
export async function call(
  ...
  if (args?.trim() === 'open') {
    ...
    const targetPath = worktreeSession
    ...
    const onInstall = (ide: IdeType) => {
      if (context.onInstallIDEExtension) {
        ...
        if (isJetBrainsIde(ide)) {
          ...
          onDone(
            `Installed extension to ${chalk.bold(toIDEDisplayName(ide))}`,
          )
        }
      }
    }
...
function IDECommandFlow({
  ...
  const [connectingIDE, setConnectingIDE] =
    useState<DetectedIDEInfo | null>(null)
  const ideClient = useAppState(s =>
    s.mcp.clients.find(c => c.name === 'ide'),
  )
