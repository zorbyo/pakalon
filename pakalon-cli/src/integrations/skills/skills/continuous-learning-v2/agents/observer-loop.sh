...
# parallel Pakalon analysis processes.
...
ANALYSIS_COOLDOWN="${Pakalon_OBSERVER_ANALYSIS_COOLDOWN:-60}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${Pakalon_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping Pakalon analysis on Windows due to known non-interactive hang issue (#295). Set Pakalon_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi

  if ! command -v Pakalon >/dev/null 2>&1; then
    echo "[$(date)] Pakalon CLI not found, skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${Pakalon_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  analysis_file="$(mktemp "${TMPDIR:-/tmp}/pakalon-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${TMPDIR:-/tmp}/pakalon-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${Pakalon_OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${Pakalon_OBSERVER_MAX_TURNS:-10}"
  exit_code=0
  ...
  Pakalon_SKIP_OBSERVE=1 Pakalon_HOOK_PROFILE=minimal Pakalon --model haiku --max-turns "$max_turns" --print < "$prompt_file" >> "$LOG_FILE" 2>&1 &
  Pakalon_pid=$!

  (
    sleep "$timeout_seconds"
    if kill -0 "$Pakalon_pid" 2>/dev/null; then
      echo "[$(date)] Pakalon analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
      kill "$Pakalon_pid" 2>/dev/null || true
    fi
  ) &
  watchdog_pid=$!
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo "[$(date)] Pakalon analysis failed (exit $exit_code)" >> "$LOG_FILE"
  fi
