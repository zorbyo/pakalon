...
#
# Detection priority:
#   1. Pakalon_PROJECT_DIR env var (if set)
...
_CLV2_HOMUNCULUS_DIR="${HOME}/.Pakalon/homunculus"
...
_clv2_detect_project() {
  ...
  # 1. Try Pakalon_PROJECT_DIR env var
  if [ -n "$Pakalon_PROJECT_DIR" ] && [ -d "$Pakalon_PROJECT_DIR" ]; then
    project_root="$Pakalon_PROJECT_DIR"
    source_hint="env"
  fi
