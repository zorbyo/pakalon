...
- AI agent (Pakalon, Cursor, Codex) has modified API routes or backend logic
...
Fix 4: Test caught it instantly on first run [OK]
```
...
```typescript
// [X] AI adds field to production path only
if (isSandboxMode()) {
...
// [OK] Both paths must return the same shape
if (isSandboxMode()) {
...
```typescript
// [X] New column added to response but not to SELECT
...
// [OK] Use SELECT * or explicitly include new columns
const { data } = await supabase
  .from("users")
  .select("*")
  .single();
...
```typescript
// [X] Error state set but old data not cleared
catch (err) {
  setError("Failed to load");
  // reservations still shows data from previous tab!
}

// [OK] Clear related state on error
catch (err) {
...
// [OK] Capture previous state and rollback on failure
const handleRemove = async (id: string) => {
...
|---|---|---|
| Sandbox/production mismatch | Assert same response shape in sandbox mode | [LARGEREDCIRCLE] High |
| SELECT clause omission | Assert all required fields in response | [LARGEREDCIRCLE] High |
| Error state leakage | Assert state cleanup on error | [LARGEYELLOWCIRCLE] Medium |
| Missing rollback | Assert state restored on API failure | [LARGEYELLOWCIRCLE] Medium |
| Type cast masking null | Assert field is not undefined | [LARGEYELLOWCIRCLE] Medium |
