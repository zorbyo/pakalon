...
description: "Patterns and architectures for autonomous Pakalon loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems."
origin: Pakalon
---
...
Patterns, architectures, and reference implementations for running Pakalon autonomously in loops. Covers everything from simple `Pakalon -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
|---------|-----------|----------|
| [Sequential Pipeline](#1-sequential-pipeline-Pakalon--p) | Low | Daily dev steps, scripted workflows |
...
| [Continuous Pakalon PR Loop](#4-continuous-Pakalon-pr-loop) | Medium | Multi-day iterative projects with CI gates |
...
## 1. Sequential Pipeline (`Pakalon -p`)

**The simplest loop.** Break daily development into a sequence of non-interactive `Pakalon -p` calls. Each call is a focused step with a clear prompt.

### Core Insight

> If you can't figure out a loop like this, it means you can't even drive the LLM to fix your code in interactive mode.

The `Pakalon -p` flag runs Pakalon non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
Pakalon -p "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
Pakalon -p "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
Pakalon -p "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
Pakalon -p "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated** — A fresh context window per `Pakalon -p` call means no context bleed between steps.
...
Pakalon -p --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
Pakalon -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
Pakalon -p --model opus "Review all changes for security issues, race conditions, and edge cases..."
```

**With environment context:**
```bash
# Pass context via files, not prompt length
echo "Focus areas: auth module, API rate limiting" > .Pakalon-context.md
Pakalon -p "Read .Pakalon-context.md for priorities. Work through them in order."
rm .Pakalon-context.md
```

**With `--allowedTools` restrictions:**
```bash
# Read-only analysis pass
Pakalon -p --allowedTools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
Pakalon -p --allowedTools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```
...
**Pakalon's built-in persistent loop.** A session-aware REPL that calls `Pakalon -p` synchronously with full conversation history.
...
1. Loads conversation history from `~/.Pakalon/claw/{session}.md`
2. Each user message is sent to `Pakalon -p` with full history as context
...
### Implementation via Pakalon Commands

Create `.Pakalon/commands/infinite.md`:
...
## 4. Continuous Pakalon PR Loop

**A production-grade shell script** that runs Pakalon in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).

### Core Loop

```
┌─────────────────────────────────────────────────────┐
│  CONTINUOUS Pakalon ITERATION                        │
│                                                     │
│  1. Create branch (continuous-Pakalon/iteration-N)   │
│  2. Run Pakalon -p with enhanced prompt              │
│  3. (Optional) Reviewer pass — separate Pakalon -p   │
│  4. Commit changes (Pakalon generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (Pakalon -p)         │
...
```bash
curl -fsSL https://raw.githubusercontent.com/AnandChowdhary/continuous-Pakalon/HEAD/install.sh | bash
```
...
```bash
# Basic: 10 iterations
continuous-Pakalon --prompt "Add unit tests for all untested functions" --max-runs 10

# Cost-limited
continuous-Pakalon --prompt "Fix all linter errors" --max-cost 5.00

# Time-boxed
continuous-Pakalon --prompt "Improve test coverage" --max-duration 8h
...
continuous-Pakalon --prompt "Add tests" --max-runs 5 --worktree tests-worker &
continuous-Pakalon --prompt "Refactor code" --max-runs 5 --worktree refactor-worker &
wait
```
...
Pakalon reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `Pakalon -p` invocations.

### CI Failure Recovery

When PR checks fail, Continuous Pakalon automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `Pakalon -p` with CI fix context
3. Pakalon inspects logs via `gh run view`, fixes code, commits, pushes
4. Re-waits for checks (up to `--ci-retry-max` attempts)

### Completion Signal

Pakalon can signal "I'm done" by outputting a magic phrase:

```bash
continuous-Pakalon \
  --prompt "Fix all bugs in the issue tracker" \
  --completion-signal "CONTINUOUS_Pakalon_PROJECT_COMPLETE" \
...
Pakalon -p "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
Pakalon -p "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  Pakalon -p "Implement $feature with TDD."

  # De-sloppify
  Pakalon -p "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  Pakalon -p "Run build + lint + tests. Fix any failures."

  # Commit
  Pakalon -p "Commit with message: feat: add $feature"
done
```
...
| Multi-day project | Yes | Maybe (continuous-Pakalon) |
...
└─ No → Is there a written spec/RFC?
         ...
         │        └─ No → Continuous Pakalon (iterative PR loop)
...
2. **Continuous Pakalon + De-Sloppify** — Add `--review-prompt` with a de-sloppify directive to each iteration.

3. **Any loop + Verification** — Use Pakalon's `/verify` command or `verification-loop` skill as a gate before commits.
...
4. **Ralphinho's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   Pakalon -p --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   Pakalon -p --model opus "Refactor the auth module to use the strategy pattern"
   ```
...
2. **No context bridge between iterations** — Each `Pakalon -p` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
...
| Continuous Pakalon | AnandChowdhary | credit: @AnandChowdhary |
| NanoClaw | Pakalon | `/claw` command in this repo |
| Verification Loop | Pakalon | `skills/verification-loop/` in this repo |
