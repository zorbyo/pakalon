...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond Pakalon's native Markdown skill loader.

## Installation

This skill ships with Everything Pakalon. No separate installation is needed when Pakalon is installed.

### Full Pakalon install

If you are working from the Pakalon repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the Pakalon diff before updating:

```bash
cd /path/to/everything-Pakalon-code
...
If you are vendoring only this skill outside the full Pakalon install, copy the reviewed file from the Pakalon repository into `~/.Pakalon/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed Pakalon commit rather than running `git pull`.

## Requirements

- Pakalon (for `/blueprint` slash command)
