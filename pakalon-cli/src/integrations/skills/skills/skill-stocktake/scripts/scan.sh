...
#
# When CWD_SKILLS_DIR is omitted, defaults to $PWD/.Pakalon/skills so the
# script always picks up project-level skills without relying on the caller.
#
# Environment:
#   SKILL_STOCKTAKE_GLOBAL_DIR   Override ~/.Pakalon/skills (for testing only;
#                                do not set in production — intended for bats tests)
#   SKILL_STOCKTAKE_PROJECT_DIR  Override project dir detection (for testing only)

set -euo pipefail

GLOBAL_DIR="${SKILL_STOCKTAKE_GLOBAL_DIR:-$HOME/.Pakalon/skills}"
CWD_SKILLS_DIR="${SKILL_STOCKTAKE_PROJECT_DIR:-${1:-$PWD/.Pakalon/skills}}"
...
OBSERVATIONS="${SKILL_STOCKTAKE_OBSERVATIONS:-$HOME/.Pakalon/observations.jsonl}"

# Validate CWD_SKILLS_DIR looks like a .Pakalon/skills path (defense-in-depth).
# Only warn when the path exists — a nonexistent path poses no traversal risk.
if [[ -n "$CWD_SKILLS_DIR" && -d "$CWD_SKILLS_DIR" && "$CWD_SKILLS_DIR" != */.Pakalon/skills* ]]; then
  echo "Warning: CWD_SKILLS_DIR does not look like a .Pakalon/skills path: $CWD_SKILLS_DIR" >&2
fi
