...
- **Code Sovereignty**: External models have **zero filesystem write access**, all modifications by Pakalon
- **Stop-Loss Mechanism**: Do not proceed to next phase until current phase output is validated
- **Planning Only**: This command allows reading context and writing to `.Pakalon/plan/*` plan files, but **NEVER modify production code**
...
```
Bash({
  command: "~/.Pakalon/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.Pakalon/.ccg/prompts/codex/analyzer.md` | `~/.Pakalon/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.Pakalon/.ccg/prompts/codex/architect.md` | `~/.Pakalon/.ccg/prompts/gemini/architect.md` |
...
**If ace-tool MCP is NOT available**, use Pakalon built-in tools as fallback:
...
1. **Codex Backend Analysis**:
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/codex/analyzer.md`
...
2. **Gemini Frontend Analysis**:
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/gemini/analyzer.md`
...
To reduce risk of omissions in Pakalon's synthesized plan, can parallel have both models output "plan drafts" (still **NOT allowed** to modify files):

1. **Codex Plan Draft** (Backend authority):
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/codex/architect.md`
...
2. **Gemini Plan Draft** (Frontend authority):
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/gemini/architect.md`
...
#### 2.4 Generate Implementation Plan (Pakalon Final Version)
...
2. Save plan to `.Pakalon/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

   ---
   **Plan generated and saved to `.Pakalon/plan/actual-feature-name.md`**
   ...
   **Please review the plan above. You can:**
   ...
   ```
   /ccg:execute .Pakalon/plan/actual-feature-name.md
   ```
   ---
...
- **First planning**: `.Pakalon/plan/<feature-name>.md`
- **Iteration versions**: `.Pakalon/plan/<feature-name>-v2.md`, `.Pakalon/plan/<feature-name>-v3.md`...
...
2. Update `.Pakalon/plan/<feature-name>.md` file
...
```bash
/ccg:execute .Pakalon/plan/<feature-name>.md
```
