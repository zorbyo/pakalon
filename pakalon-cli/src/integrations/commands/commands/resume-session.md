---
description: Load the most recent session file from ~/.Pakalon/sessions/ and resume work with full context from where the last session ended.
---
...
- Any time you have a session file and want Pakalon to fully absorb it before proceeding

## Usage

```
/resume-session                                                      # loads most recent file in ~/.Pakalon/sessions/
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session ~/.Pakalon/sessions/2024-01-15-session.tmp           # loads a specific legacy-format file
/resume-session ~/.Pakalon/sessions/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
```
...
1. Check `~/.Pakalon/sessions/`
2. Pick the most recently modified `*-session.tmp` file
3. If the folder does not exist or has no matching files, tell the user:
   ```
   No session files found in ~/.Pakalon/sessions/
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.Pakalon/sessions/` for files matching
...
CURRENT STATE:
[OK] Working: [count] items confirmed
[ANTICLOCKWISEDOWNWARDSAND] In Progress: [list files that are in progress]
[SPIRALNOTEPAD] Not Started: [list planned but untouched]
...
```
SESSION LOADED: /Users/you/.Pakalon/sessions/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
...
CURRENT STATE:
[OK] Working: 3 items (register endpoint, JWT generation, password hashing)
[ANTICLOCKWISEDOWNWARDSAND] In Progress: app/api/auth/login/route.ts (token works, cookie not set yet)
[SPIRALNOTEPAD] Not Started: middleware.ts, app/login/page.tsx

WHAT NOT TO RETRY:
[X] Next-Auth — conflicts with custom Prisma adapter, threw adapter error on every request
[X] localStorage for JWT — causes SSR hydration mismatch, incompatible with Next.js
