...
```bash
python3 "${Pakalon_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" promote [instinct-id] [--force] [--dry-run]
```

Or if `Pakalon_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.Pakalon/skills/continuous-learning-v2/scripts/instinct-cli.py promote [instinct-id] [--force] [--dry-run]
```
...
4. Write promoted instincts to `~/.Pakalon/homunculus/instincts/personal/` with `scope: global`
