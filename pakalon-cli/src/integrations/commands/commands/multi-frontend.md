...
# New session call
Bash({
  command: "~/.Pakalon/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.Pakalon/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Gemini |
|-------|--------|
| Analysis | `~/.Pakalon/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.Pakalon/.ccg/prompts/gemini/architect.md` |
| Review | `~/.Pakalon/.ccg/prompts/gemini/reviewer.md` |
...
Pakalon synthesizes plan, save to `.Pakalon/plan/task-name.md` after user approval.
...
4. Pakalon handles all code writes and file operations
