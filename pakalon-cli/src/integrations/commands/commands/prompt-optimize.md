---
description: Analyze a draft prompt and output an optimized, pakalon-enriched version ready to paste and run. Does NOT execute the task — outputs advisory analysis only.
---

# /prompt-optimize

Analyze and optimize the following prompt for maximum Pakalon leverage.

## Your Task

Apply the **prompt-optimizer** skill to the user's input below. Follow the 6-phase analysis pipeline:

0. **Project Detection** — Read Pakalon.md, detect tech stack from project files (package.json, go.mod, pyproject.toml, etc.)
...
3. **Pakalon Component Matching** — Map to specific skills, commands, agents, and model tier
...
- Present diagnosis, recommended Pakalon components, and an optimized prompt using the Output Format from the prompt-optimizer skill
