---
description: Manage Pakalon session history, aliases, and session metadata.
---

# Sessions Command

Manage Pakalon session history - list, load, alias, and edit sessions stored in `~/.Pakalon/sessions/`.
...
```bash
node -e "
const sm = require((process.env.Pakalon_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.Pakalon'))+'/scripts/lib/session-manager');
const aa = require((process.env.Pakalon_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.Pakalon'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.Pakalon/sessions/' + session.filename);
console.log('');
...
if (result.success) {
  console.log('[OK] Alias created: ' + aliasName + ' → ' + session.filename);
} else {
...
if (result.success) {
  console.log('[OK] Alias removed: ' + aliasName);
} else {
  console.log('[X] Error: ' + result.error);
  process.exit(1);
}
...
- Sessions are stored as markdown files in `~/.Pakalon/sessions/`
- Aliases are stored in `~/.Pakalon/session-aliases.json`
