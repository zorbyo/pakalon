...
- Multi-model collaboration: Codex (backend) + Gemini (frontend) + Pakalon (orchestration)
...
- **Pakalon (self)** – Orchestration, planning, execution, delivery
...
```bash
node scripts/orchestrate-worktrees.js .Pakalon/plan/workflow-e2e-test.json --execute
```
...
**Pakalon Synthesis**: Adopt Codex backend plan + Gemini frontend plan, save to `.Pakalon/plan/task-name.md` after user approval.
...
2. External models have **zero filesystem write access**, all modifications by Pakalon
