...
Multi-model collaborative execution - Get prototype from plan → Pakalon refactors and implements → Multi-model audit and delivery.
...
Bash({
  command: "~/.Pakalon/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Implementation | `~/.Pakalon/.ccg/prompts/codex/architect.md` | `~/.Pakalon/.ccg/prompts/gemini/frontend.md` |
| Review | `~/.Pakalon/.ccg/prompts/codex/reviewer.md` | `~/.Pakalon/.ccg/prompts/gemini/reviewer.md` |
...
1. **Identify Input Type**:
   - Plan file path (e.g., `.Pakalon/plan/xxx.md`)
...
1. Call Gemini (use `~/.Pakalon/.ccg/prompts/gemini/frontend.md`)
...
1. Call Codex (use `~/.Pakalon/.ccg/prompts/codex/architect.md`)
2. Input: Plan content + retrieved context + target files
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
**Pakalon as Code Sovereign executes the following steps**:
...
1. **Codex Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/codex/reviewer.md`
...
2. **Gemini Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.Pakalon/.ccg/prompts/gemini/reviewer.md`
   - Input: Changed Diff + target files
...
1. **Code Sovereignty** – All file modifications by Pakalon, external models have zero write access
