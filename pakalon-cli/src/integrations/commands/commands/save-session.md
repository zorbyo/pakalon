---
description: Save current session state to a dated file in ~/.Pakalon/sessions/ so work can be resumed in a future session with full context.
---
...
- End of a work session before closing Pakalon
...
Create the canonical sessions folder in the user's Pakalon home directory:

```bash
mkdir -p ~/.Pakalon/sessions
```

### Step 3: Write the session file

Create `~/.Pakalon/sessions/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
| File              | Status         | Notes                      |
| ----------------- | -------------- | -------------------------- |
| `path/to/file.ts` | [OK] Complete    | [what it does]             |
| `path/to/file.ts` | [ANTICLOCKWISEDOWNWARDSAND] In Progress | [what's done, what's left] |
| `path/to/file.ts` | [X] Broken      | [what's wrong]             |
| `path/to/file.ts` | [SPIRALNOTEPAD] Not Started | [planned but not touched]  |
...
| File                             | Status         | Notes                                           |
| -------------------------------- | -------------- | ----------------------------------------------- |
| `app/api/auth/register/route.ts` | [OK] Complete    | Works, tested                                   |
| `app/api/auth/login/route.ts`    | [ANTICLOCKWISEDOWNWARDSAND] In Progress | Token generates but not setting cookie yet      |
| `lib/auth.ts`                    | [OK] Complete    | JWT helpers, all tested                         |
| `middleware.ts`                  | [SPIRALNOTEPAD] Not Started | Route protection, needs cookie read logic first |
| `app/login/page.tsx`             | [SPIRALNOTEPAD] Not Started | UI not started                                  |
...
- The file is meant to be read by Pakalon at the start of the next session via `/resume-session`
- Use the canonical global session store: `~/.Pakalon/sessions/`
