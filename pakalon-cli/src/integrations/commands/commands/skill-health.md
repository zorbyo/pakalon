...
```bash
node "${Pakalon_PLUGIN_ROOT}/scripts/skills-health.js" --dashboard
```

For a specific panel only:

```bash
node "${Pakalon_PLUGIN_ROOT}/scripts/skills-health.js" --dashboard --panel failures
```

For machine-readable output:

```bash
node "${Pakalon_PLUGIN_ROOT}/scripts/skills-health.js" --dashboard --json
```
