...
- **Rules are system-injected**: `.Pakalon/rules/*.md` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

```bash
Pakalon /mail                    # Email-only triage
Pakalon /slack                   # Slack-only triage
Pakalon /today                   # All channels + calendar + todo
Pakalon /schedule-reply "Reply to Sarah about the board meeting"
```

## Prerequisites

- [Pakalon](https://docs.anthropic.com/en/docs/Pakalon-code)
