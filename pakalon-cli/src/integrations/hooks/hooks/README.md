...
Hooks are event-driven automations that fire before or after Pakalon tool executions. They enforce code quality, catch mistakes early, and automate repetitive checks.

## How Hooks Work

```
User request → Pakalon picks a tool → PreToolUse hook runs → Tool executes → PostToolUse hook runs
```
...
- **Stop** hooks run after each Pakalon response.
...
| **Doc file warning** | `Write` | Warns about non-standard `.md`/`.txt` files (allows README, Pakalon, CONTRIBUTING, CHANGELOG, LICENSE, SKILL, docs/, skills/); cross-platform path handling | 0 (warns) |
| **Strategic compact** | `Edit\|Write` | Suggests manual `/compact` at logical intervals (every ~50 tool calls) | 0 (warns) |
| **InsAIts security monitor (opt-in)** | `Bash\|Write\|Edit\|MultiEdit` | Optional security scan for high-signal tool inputs. Disabled unless `Pakalon_ENABLE_INSAITS=1`. Blocks on critical findings, warns on non-critical, and writes audit log to `.insaits_audit_session.jsonl`. Requires `pip install insa-its`. [Details](../scripts/hooks/insaits-security-monitor.py) | 2 (blocks critical) / 0 (warns) |
...
Remove or comment out the hook entry in `hooks.json`. If installed as a plugin, override in your `~/.Pakalon/settings.json`:
...
export Pakalon_HOOK_PROFILE=standard

# Disable specific hook IDs (comma-separated)
export Pakalon_DISABLED_HOOKS="pre:bash:tmux-reminder,post:edit:typecheck"
```
...
process.stdin.on('end', () => {
  ...
  console.error('[Hook] Warning message shown to Pakalon');
