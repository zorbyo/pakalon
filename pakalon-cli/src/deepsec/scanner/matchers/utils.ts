...
import type { CandidateMatch } from "../../core/types.js";
...
/**
 * Create a simple matcher from regex patterns
 */
export function createRegexMatcher(
  ...
  filePatterns: string[],
  patterns: { regex: RegExp; label: string }[],
): { slug: string; description: string; noiseTier: "low" | "normal" | "high"; filePatterns: string[]; match: (content: string, filePath: string) => CandidateMatch[] } {
  return {
    slug,
    description,
    noiseTier,
