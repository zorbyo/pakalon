 ...
 * Sends email notifications for billing-related events
 */
import type { BillingInfo, BillingPlan, BillingUsage, BillingEmailPayload, BillingInvoice } from "./types.js";
...
export async function sendBillingEmail(payload: BillingEmailPayload): Promise<void> {
  try {
    ...
    if (!response.ok) {
      console.warn(`Billing email failed: ${response.statusText}`);
    }
  } catch (error) {
    // Fallback to console notification if email endpoint unavailable
    console.log(`[Billing ${payload.type}] Email notification could not be sent:`, error);
  }
}
...
export async function sendBillingReminder(
  ...
  await sendBillingEmail({
    ...
    email,
    plan: { id: "post-paid", name: "Post-Paid", price: amount, features: [], tokenLimit: Infinity, requestLimit: Infinity },
...
export async function sendPaymentReceipt(email: string, invoice: BillingInvoice): Promise<void> {
  await sendBillingEmail({
    ...
    email,
    plan: { id: "post-paid", name: "Post-Paid", price: invoice.totalUsd, features: [], tokenLimit: Infinity, requestLimit: Infinity },
